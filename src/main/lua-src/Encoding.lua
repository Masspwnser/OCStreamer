...
-- Cache how the 255 braille characters map to unicode
for i = 0, 255 do
    brailleChars[i] = string.char(0xE2, 0xA0 + bit32.rshift(bit32.band(i, 0xC0), 6), 0x80 + bit32.band(i, 0x3F))
end

-- I swear to god there was a reason for making this so complicated... i've just forgotten it
function Encoding.readPixelData(handle)
    ...
    for chunk in handle do
        ...
        if pixelPos == 1 then
            picture[1] = string.byte(chunk, 1) + bit32.lshift(string.byte(chunk, 2), 8)
		...
		while chunkPos <= chunkSize do
			local byte = string.byte(chunk, chunkPos)
			local pixelType = math.fmod(pixelPos, 3)
			if pixelType <= 1 then
				picture[pixelPos] = colors[byte + 1]
			else
				picture[pixelPos] = brailleChars[byte]
			end
			pixelPos = pixelPos + 1
			chunkPos = chunkPos + 1
		end
