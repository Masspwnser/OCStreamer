...
function Functions.readPixelData(file, width, height)
	picture[1] = width
	picture[2] = height
	local numPixels = width * height;
	for i = 3, numPixels*3, 3 do
		picture[i] = to24Bit(string.byte(file:read(1)))
		picture[i+1] = to24Bit(string.byte(file:read(1)))
		picture[i+2] = readUnicodeChar(string.byte(file:read(1)))
	end
	return picture
end
