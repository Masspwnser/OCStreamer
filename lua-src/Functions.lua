...
function Functions.initialize()
    ...
    for i = 0, 255 do
        brailleChars[i] = string.char(0xE2, 0xA0 + bit32.rshift(bit32.band(i, 0xC0), 6), 0x80 + bit32.band(i, 0x3F))
    end
end
    ...
    return {
        ...
        getPixel = function(self, x, y)
            ...
            if row then
                local pixel = row[x]
                if pixel then
                    return pixel.bg, pixel.fg, pixel.char
                end
            end
            return 0x000000, 0xFFFFFF, " "
        end,
        setPixel = function(self, x, y, bg, fg, char)
            if not self.pixels[y] then
                self.pixels[y] = {}
            end
            self.pixels[y][x] = {bg = bg, fg = fg, char = char}
        end
    }
end

function Functions.readPixelData(handle)
    ...
    for chunk in handle do
        if firstChunk then
            local width = string.byte(chunk, 1) + bit32.lshift(string.byte(chunk, 2), 8)
            local height = string.byte(chunk, 3) + bit32.lshift(string.byte(chunk, 4), 8)
            image = createImage(width, height)
            
            -- Process remaining bytes in first chunk
            local pos = 5
            while pos <= #chunk do
                if pos + 2 <= #chunk then
                    local bg = palette[string.byte(chunk, pos) + 1]
                    local fg = palette[string.byte(chunk, pos + 1) + 1]
                    local char = brailleChars[string.byte(chunk, pos + 2)]
                    image:setPixel(x, y, bg, fg, char)
